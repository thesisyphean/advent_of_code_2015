...
fn main() {
    ...
    // TODO: the second arm will never be reached
    // because reading the input file will throw an error first
    match solve(problem, part).expect("Error") {
        Some(solution) => println!("Solution: {solution}"),
        _ => println!("Problem {problem} part {part} does not exist."),
    }
}
...
fn solve(problem: u32, part: u32) -> io::Result<Option<u32>> {
    let input = get_input(&format!("inputs/day_{problem}.txt"))?;

    // could have written a macro, but it was much quicker
    // to write a python script to write the lines to this file
